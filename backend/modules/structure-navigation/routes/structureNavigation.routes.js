...
const ctrl    = require('../controller/structureNavigation.controller');
...
router.post('/analyze', optionalAuth, async (req, res, next) => {
  try {
    ...
    await ctrl.analyzeController(req, res, next);
  } catch (err) { next(err); }
});

router.get('/report/:id',   optionalAuth, ctrl.getReportController);
router.get('/download/:id', requireAuth,  ctrl.downloadReportController);
