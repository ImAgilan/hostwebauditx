...
const ctrl    = require('../controller/accessibility.controller');
...
router.post('/analyze', optionalAuth, async (req, res, next) => {
  try {
    ...
    /* ── Hook res.json — must be synchronous, NOT async ── */
    if (req.user) {
      const originalJson = res.json.bind(res);
      res.json = (body) => {                          // ← NO async here
        if (body && body.success) {
          incrementAuditCount(req.user)               // ← NO await, fire and forget
